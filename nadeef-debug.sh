...
{
  cmd="java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=1044 $JAVA_ARGS -cp out/bin/*:examples/:out/test qa.qcri.nadeef.console.Console"
  exec $cmd
}

function run_dashboard()
{
  cmd="java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=1044 $JAVA_ARGS -cp out/bin/*:. qa.qcri.nadeef.web.NadeefStart"
  exec $cmd
}
